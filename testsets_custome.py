        ...
        for idx in range(len(self.im_list)):
            ...
            with torch.no_grad():
                with model.ema_scope():
                    # Form condition tensor and define shape of latent representation
                    ...
                    # Run sampling and get denoised latent representation
...
class CustomData(TripletTestSet):
    def __init__(self, db_dir):
        ...
        # Dynamically generate sequence names for multiple sequences
        self.im_list = [f'sequence{i + 1}' for i in range(39)]  # Adjust this range based on the number of sequences you have

        # Initialize lists for input and ground truth frames
        self.input0_list = []
        self.input1_list = []
        self.gt_list = []

        # Loop through each sequence
        for sequence_name in self.im_list:
            # Define the input frame paths
            input_frame1_path = join(db_dir, 'input', sequence_name, '1.png')
            input_frame2_path = join(db_dir, 'input', sequence_name, '3.png')
            
            # Load the first input frame
            if exists(input_frame1_path):
                self.input0_list.append(self.transform(Image.open(input_frame1_path).convert('RGB')).cuda().unsqueeze(0))
            else:
                print(f"Warning: Input frame {input_frame1_path} does not exist.")

            # Load the second input frame
            if exists(input_frame2_path):
                self.input1_list.append(self.transform(Image.open(input_frame2_path).convert('RGB')).cuda().unsqueeze(0))
            else:
                print(f"Warning: Input frame {input_frame2_path} does not exist.")

            # Define the corresponding ground truth path
            gt_frame_path = join(db_dir, 'gt', sequence_name, '2.png')  # Adjust naming as needed
            if exists(gt_frame_path):
                self.gt_list.append(self.transform(Image.open(gt_frame_path).convert('RGB')).cuda().unsqueeze(0))
            else:
                print(f"Warning: Ground truth frame {gt_frame_path} does not exist.")

        # Debugging prints
        print(f"Loaded {len(self.input0_list)} input frames, {len(self.input1_list)} next input frames, and {len(self.gt_list)} ground truth frames.")
...
class Middlebury_others(TripletTestSet):
    def __init__(self, db_dir):
        ...
        for item in self.im_list:
            self.input0_list.append(self.transform(Image.open(join(db_dir, 'input', item, 'frame10.png')).convert('RGB')).cuda().unsqueeze(0))
            self.input1_list.append(self.transform(Image.open(join(db_dir, 'input', item, 'frame11.png')).convert('RGB')).cuda().unsqueeze(0))
            self.gt_list.append(self.transform(Image.open(join(db_dir, 'gt', item, 'frame10i11.png')).convert('RGB')).cuda().unsqueeze(0))
...
class Ucf(TripletTestSet):
    def __init__(self, db_dir):
        ...
        self.gt_list = []
        for item in self.im_list:
            self.input0_list.append(self.transform(Image.open(join(db_dir, item, 'frame_00.png')).convert('RGB')).cuda().unsqueeze(0))
            self.input1_list.append(self.transform(Image.open(join(db_dir, item, 'frame_02.png')).convert('RGB')).cuda().unsqueeze(0))
            self.gt_list.append(self.transform(Image.open(join(db_dir, item, 'frame_01_gt.png')).convert('RGB')).cuda().unsqueeze(0))
...
class Snufilm(TripletTestSet):
    def __init__(self, db_dir, mode):
        ...
        for i, triplet in enumerate(triplet_list, 1):
            ...
            lst = triplet.split(' ')
            self.input0_list.append(self.transform(Image.open(join(db_dir, lst[0])).convert('RGB')).cuda().unsqueeze(0))
            self.input1_list.append(self.transform(Image.open(join(db_dir, lst[2])).convert('RGB')).cuda().unsqueeze(0))
            self.gt_list.append(self.transform(Image.open(join(db_dir, lst[1])).convert('RGB')).cuda().unsqueeze(0))
...
class Ucf101_triplet:
    def __init__(self, db_dir):
        ...
        self.gt_list = []
        for item in self.im_list:
            self.input3_list.append(self.transform(Image.open(join(db_dir, item, 'frame1.png')).convert('RGB')).cuda().unsqueeze(0))
            self.input5_list.append(self.transform(Image.open(join(db_dir, item, 'frame2.png')).convert('RGB')).cuda().unsqueeze(0))
            self.gt_list.append(self.transform(Image.open(join(db_dir, item, 'framet.png')).convert('RGB')).cuda().unsqueeze(0))
